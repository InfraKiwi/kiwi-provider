...
import { ciRootDir, ciSrcDir } from './common';
...
const docsNavBasePath = path.join(ciRootDir, 'hack', 'docs');
...
async function genRegistriesEntries() {
  ...
  const pathGlob = path.resolve(ciSrcDir, '**/registry.ts').replaceAll(path.sep, '/');
  ...
  for (const key in loadAllGroups) {
    ...
    const loadAllCentralFile = path.resolve(ciRootDir, 'src', 'util', `loadAllRegistryEntries${fileSuffix}.gen.ts`);
...
const genSchemaFolders = async () => {
  ...
  const pathGlob = path.resolve(ciSrcDir, '**/*schema.ts').replaceAll(path.sep, '/');
...
async function fixGenImports() {
  const files = globSync(path.resolve(ciSrcDir, '**/*schema.gen.ts').replaceAll(path.sep, '/'));
...
interface ImportGroup {
  // e.g. loadAll.kiwiProvider.gen.ts => .kiwiProvider
...
async function genSystemInformationSchema(fnDescriptions: Record<string, string> = {}) {
  const typesFile = path.resolve(ciRootDir, path.normalize('node_modules/systeminformation/lib/index.d.ts'));
  const out = path.resolve(ciRootDir, path.normalize('src/modules/info/schema.ts'));
...
async function genSystemInformationReadme() {
  const siReadme = path.resolve(ciRootDir, path.normalize('node_modules/systeminformation/README.md'));
  const readmeTemplate = path.resolve(ciRootDir, path.normalize('src/modules/info/README.template.md'));
  const out = path.resolve(ciRootDir, path.normalize('src/modules/info/README.md'));
