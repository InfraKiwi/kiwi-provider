...
import { ConfigProviderHeaderHostname } from '../configProvider/configProvider.schema';
...
export class Agent {
  ...
  #newApiClient(context: ContextLogger, prefixText: string): Axios {
    const client = axios.create({
      baseURL: `${this.#config.configProviderUrl}/host`,
      headers: { [ConfigProviderHeaderHostname]: this.#hostname },
    });
    ...
    await childLogger.wrapContext(
      ...
      async (_context) => {
        try {
          ...
          const configForHostname: ConfigProviderRouteGetConfigForHostnameResultInterface = (
