...
export const ModuleInfoSchema = moduleRegistryEntryFactory.createJoiEntrySchema(
  ...
  Joi.object({
    ...
    versions: Joi.object({ apps: Joi.string() }).description(`version information (kernel, ssl, node, ...)
...
(string, comma separated)`),
    ...
    cpuTemperature: Joi.boolean().description(`CPU temperature in C (if supported)`),
    ...
    graphics: Joi.boolean().description(`arrays of graphics controllers and displays`),
    fsSize: Joi.object({ drive: Joi.string() }).description(`returns array of mounted file systems
drive param is optional`),
    fsOpenFiles: Joi.boolean().description(`count max/allocated file descriptors`),
...
raids and roms`),
    ...
    networkInterfaceDefault: Joi.boolean().description(`get name of default network interface`),
    networkGatewayDefault: Joi.boolean().description(`get default network gateway`),
...
only the default interface`),
    networkStats: Joi.object({ ifaces: Joi.string() }).description(`current network stats of given interfaces
...
Pass '*' for all interfaces`),
    networkConnections: Joi.boolean().description(`current network network connections
returns an array of all connections`),
    inetChecksite: Joi.object({ url: Joi.string().required() }).description(`response-time (ms) to fetch given URL`),
    inetLatency: Joi.object({ host: Joi.string() }).description(`response-time (ms) to external resource
host parameter is optional (default 8.8.8.8)`),
    wifiNetworks: Joi.boolean().description(`array of available wifi networks`),
    wifiInterfaces: Joi.boolean().description(`array of detected wifi interfaces`),
    wifiConnections: Joi.boolean().description(`array of active wifi connections`),
...
pass "*" for ALL processes (linux/win only)`),
    services: Joi.object({ serviceName: Joi.string().required() }).description(`pass comma separated string of services
pass "*" for ALL services (linux/win only)`),
    dockerInfo: Joi.boolean().description(`returns general docker info`),
    dockerImages: Joi.object({ all: Joi.boolean() }).description(`returns array of top level/all docker images`),
    dockerContainers: Joi.object({ all: Joi.boolean() }).description(`returns array of active/all docker containers`),
    dockerContainerStats: Joi.object({ id: Joi.string() }).description(`statistics for specific containers
container IDs: space or comma separated,
pass '*' for all containers`),
    dockerContainerProcesses: Joi.object({ id: Joi.string() }).description(`array of processes inside a container`),
    dockerVolumes: Joi.boolean().description(`volume name`),
    dockerAll: Joi.boolean().description(`list of all containers including their stats
and processes in one single array`),
    vboxInfo: Joi.boolean().description(`returns array general virtual box info`),
